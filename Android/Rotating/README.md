...
![예시](https://github.com/kps990515/ProgrammingStudy/blob/master/Android/Rotating/%EC%98%88%EC%8B%9C.png)

### [전체코드](https://github.com/kps990515/ProgrammingStudy/blob/master/Android/Rotating/app/src/main/java/org/andriodtown/rotating/MainActivity.java)
...
#### move
- count를 받아서 버튼을 누를 때 마다 기능이 변경되도록
- 움직이는 버튼마다 X,Y축 이동하는 기능 + 회전 기능구현
- count=0 이면 버튼이 돌아가면서 일정 거리만큼 이동
- count=1 이면 버튼이 돌아가면서 원래자리로 복귀
```java
    public void move()
    {
        if(count==0) {
